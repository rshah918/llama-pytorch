...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        value = torch.repeat_interleave(value, repeats=self.num_query_heads//self.num_kv_heads, dim=1)  # (seq_len, num_query_heads, head_dim) since num_kv_heads is equal to num_query heads now
        ...
        attention_scores = torch.matmul(query,key.transpose(1,2)) / math.sqrt(self.head_dim)  # (num_query_heads, seq_len, seq_len)
        mask = torch.tril(torch.ones((x.shape[0], x.shape[0]), device=query.device)).unsqueeze(0)  # (1, seq_len, seq_len)
...
class DecoderLayer(nn.Module):
    def __init__(self, num_attention_heads, head_dim, num_kv_heads, len_embedding, len_sequence, intermediate_size, device):
        super(DecoderLayer, self).__init__()
        self.grouped_query_attention = GroupedQueryAttention(num_attention_heads, head_dim, num_kv_heads, len_embedding, len_sequence, device=device)
...
class Decoder(nn.Module):
    def __init__(self, vocab_size, num_decoder_layers, num_attention_heads, num_kv_heads, len_embedding, len_sequence, intermediate_size, device):
        super(Decoder, self).__init__()
        self.device = device
        # Chain together multiple decoder layers.
        self.decoder_layers = nn.Sequential(*[DecoderLayer(num_attention_heads, len_embedding // num_attention_heads, num_kv_heads, len_embedding, len_sequence, intermediate_size, device) for i in range(num_decoder_layers)])
